...
const tabsNewsBody = document.querySelector('.page__about');

tabsNewsBody.addEventListener("click", function (e) {
	const targetElem = e.target;
	if (targetElem.classList.contains('block-about__button')) {
...
const pagePopular = document.querySelector('.page__popular');

pagePopular.addEventListener("click", function (e) {
	const targetElem = e.target;
	if (targetElem.classList.contains('column-popular__button')) {
		_slideToggle(targetElem.previousElementSibling, 500);
		targetElem.classList.toggle('_active');
		e.preventDefault();
	}
...
const pageLatest = document.querySelector('.page__latest');

pageLatest.addEventListener("click", function (e) {
	const targetElem = e.target;
	if (targetElem.classList.contains('column-latest__button')) {
		_slideToggle(targetElem.previousElementSibling, 500);
		targetElem.classList.toggle('_active');
		e.preventDefault();
	}
...
//=============================================================================
/*
let button = document.querySelector(".form-content__button");
if (button != null) {
	let delay = 500;
	let attention = document.querySelector(".form-content__attention");
	button.addEventListener("click", function (e) {


		body_lock(delay);
		button.classList.add("_active");
		attention.classList.add("_active");
		setTimeout(() => {
			button.classList.remove('_active');
			attention.classList.remove('_active');
...
let formAttention = document.querySelector('.form-content__attention');
let formFree = document.querySelector('.form-content__free');

formBtn.addEventListener("click", someClick);
function someClick() {

	formBtn.classList.toggle('_active');
	formFree.classList.toggle('_active');
	formAttention.classList.toggle('_active');
...
let contentAttention = document.querySelector('.content__attention');
...
const latestContainer = document.querySelector('.latest__container');

latestContainer.addEventListener("click", function (e) {
	const targetElem = e.target;

	if (targetElem.classList.contains('latest__button')) {
		if (!targetElem.classList.contains('_loading')) {
			targetElem.classList.add('_loading');
			readMoreQuestions(targetElem);
		}
		e.preventDefault();
	}
});

async function readMoreQuestions(targetElem) {
	const response = await fetch('data/faq.html');
	if (response.ok) {
		const text = await response.text();
		const investorCounter = document.querySelector('.latest__block');
		investorCounter.insertAdjacentHTML('beforeend', text);
...
const valueSpan = document.querySelector('.textarea__value span');
...
//======================================================================
function sliders_bild_callback(params) { }
let sliderPageSlider = document.querySelector('.slider-page__slider');
if (sliderPageSlider) {


	new Swiper(sliderPageSlider, {
		observer: true,
		observeParents: true,
		slidesPerView: 4,
		spaceBetween: 10,
